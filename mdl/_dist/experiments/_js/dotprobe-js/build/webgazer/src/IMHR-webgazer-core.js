(function(window, undefined) {
    ...
    webgazer.params.camConstraints = webgazer.params.camConstraints || webcam_rtc;
    ...
    */
    ...
    webgazer.begin = function(onFail) {
        ...
        onFail = onFail || function(){
			subject.webcam_used = false;
			console.log('%c8-2.webgazer failed','color: red');
		};
        ...
        if(navigator.getUserMedia != null){
            ...
            navigator.getUserMedia(options,
                    function(stream){
						//console.log(stream)
						//prevent poorly functioning home webcam from running for future sessions
						//check if not first session, webcam failed before, and user is at home
						if ((pid.session > 0) && (pid.eyetracking == 'false') && (pid.location == 'home')){
							console.log('%csession: %s; WebcamFailedBefore: %s; location: %s','color: blue', pid.session, pid.eyetracking, pid.location);
							console.log('%cwebcam failed last session','color: red');
							WebcamMessage = "PreviousFail"; subject.webcam_used = false;
							onFail(); videoElement = null;
						} else {
							console.log('%csession: %s; WebcamPreviously: %s; location: %s','color: blue', pid.session, pid.eyetracking, pid.location);
							console.log('%cwebcam success','color: green');
							WebcamMessage = "Success"; subject.webcam_used = true;
							init(window.URL.createObjectURL(stream));
						}
                    },
                    function(e){
						console.log('%c8-1.webcam message: %s','color: red',e.name);
						WebcamMessage = e.name; subject.webcam_used = false;
						onFail(); videoElement = null;
					});
        }
        if (!navigator.getUserMedia) {
			console.log('webgazer fail');
			WebcamMessage = "getUserMediaFailed"; subject.webcam_used = false;
        ...
        if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.chrome){
			console.log('webgazer fail');
			WebcamMessage = "NotHttps"; subject.webcam_used = false;
