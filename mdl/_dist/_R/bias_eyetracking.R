...
data_path <- "~/Desktop/mdl-R33-analysis/output"
load_path <- paste(data_path, "process/data/eyetracking", sep = "/")
tlbs_path <- paste(data_path, "tlbs/eyetracking", sep = "/")
summary_path <- paste(data_path, "bias", sep = "/")
n_cores <- 4
n_trial <- 198
...
process_dotprobe <- function(path, save_path, n_trial){
  df <- read_csv(path, na = c(".", ""), guess_max = 999999,
                 col_types = cols_only(participant = col_character(), #participant
                                       ...
                                       sg_class = col_number(), #fixation, saccade, etc.
  ...
  #filter for eyetracking participants only
  df <- df %>% 
    filter(type == 'eyetracking')
  ...
  itrak_data <- df %>% 
    ...
    mutate(gaze_direction = factor(gaze_direction, levels = c("1", "2"), labels = c("Left", "Right")))
...
df_bias <- function(file_name, tlbs_folder, groupby){
  df <- read_csv(file_name, col_types = cols_only(id = col_character(),
                                           trial = col_double(),
                                           trialType = col_character(),
...
write_csv(bias_summary, paste(summary_path, "/eyetracking_bias", ".csv", sep = ""))
