...
scale = 1.0
coordinates = None
shape = 'straight' #p, h, c, rt, or str #https://docs.opencv.org/3.1.0/dd/d49/tutorial_py_contour_features.html
dpi = 300
save_data = True
...
class roi():
    def __init__(self, source=None, destination=None, shape='box', **kwargs):
        ...
        **Resources**
        ...
        #center
        self.cx = screensize[0]/2
        self.cy = screensize[1]/2
        ...
        # save csv
        self.save['data'] = True if save_data else False
        # save contour images
        self.save['contours'] = True if save_contour_image else False
        # save raw images
        self.save['raw'] = True if save_contour_image else False
        # save level
...
#----config
#----parameters
#!!! convert sllf['cx'] to self.cx when putting everything in function
save = {}
# delimiter
sllf['delimiter'] = ';' if 'delimiter' not in kwargs else kwargs['delimiter']
# screensize
sllf['screensize'] = [1920, 1080] if 'screensize' not in kwargs else kwargs['screensize']
# scale
sllf['scale'] = 1.0 if 'scale' not in kwargs else kwargs['scale']
#center
sllf['cx'] = screensize[0]/2
sllf['cy'] = screensize[1]/2
# coordinates
sllf['coordinates'] = [sllf['cx'], sllf['cy']] if 'coordinates' not in kwargs else kwargs['coordinates']
# shape
sllf['shape'] = shape if shape in ['polygon', 'hull', 'circle', 'rotate', 'straight'] else 'straight'
# dpi
sllf['dpi'] = 300 if 'dpi' not in kwargs else kwargs['dpi']
# save csv
sllf['save']['data'] = True if save_data else False
# save contour images
sllf['save']['contours'] = True if save_contour_image else False
# save raw images
sllf['save']['raw'] = True if save_contour_image else False
# save level
sllf['save']['level'] = level
#--------------------------------------------------------------------------------------------------------------------init end
...
for file in directory:
    ...
    for layer in psd:
        ...
        if Path(layer.name).stem == imagename:
            ...
            # prepare metadata
            metadata = pd.DataFrame(data=(item.split("=") for item in layer.name.split(sllf.delimiter)),columns=['key','value'])
            ...
            if ((sllf.config['save']['level'] == 'both') or (sllf.config['save']['level'] == 'roi')):
                #----save roi contour image
                if sllf.config['save']['contours']:
                    #----from source
                    ...
                    ## check if path exists
                    _folder = '%s\\output\\roi\\contours\\final\\%s'%(destination, shape)
                    if not os.path.exists(_folder):
                        os.makedirs(_folder)
                    ## save
                    plt.savefig('%s\\%s_%s_source.png'%(_folder, imagename, roiname), dpi=dpi)
                    plt.close()
                    ...
                    # create blank image
                    _img = Image.new("RGBA", (sllf.config['width'], sllf.config['height']), (255, 255, 255))
                    ax.imshow(_img)
                    # draw bounds
                    ...
                    ## check if path exists
                    _folder = '%s\\output\\roi\\contours\\debug\\%s'%(destination, shape)
                    if not os.path.exists(_folder):
                        os.makedirs(_folder)
                    ## save
                    plt.savefig('%s\\%s_%s_source.png'%(_folder, imagename, roiname), dpi=dpi)
    ...
    #----save all roi data
    if ((sllf.save['level'] == 'both') or (sllf.save['level'] == 'stimulus')):
        #----save all roi image
        if sllf.save['raw']:
        ...
        ## to csv
        df.to_csv("%s/output/stim/data/%s_bounds.csv"%(destination, imagename), index=False)
        #coord
