...
class Connect(object):
    ...
    def __init__(self, window, edfname):
        ...
        self.tracker.sendMessage("screen_pixel_coords =  0 0 %d %d" %(w - 1, h - 1))
...
#####delete-end
    ...
    def fix_check(self, size, ftime, button):
        ...
        :param size: Length of one side of box in pixels
        :type size: float or int
        ...
        xbdr = [((w/2)-fixbar), ((w/2)+fixbar)]
        ybdr = [((h/2)-fixbar),((h/2)+fixbar)]

        # Set status message & Draw box
        self.set_status('Fixation Check')
        self.tracker.sendCommand("draw_box  %d %d %d %d" %(xbdr[0],ybdr[0],xbdr[1],ybdr[1]))
