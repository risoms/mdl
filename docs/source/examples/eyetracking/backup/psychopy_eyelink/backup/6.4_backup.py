...
class libeyelink():
    ...
    def pupil_cue(self,lst,p_min,p_max):
        global p_ma
...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        while Fixation:
            ...
            while FixationClock.getTime() < 2:
                ...
                if t >= 1.75: #baseline duration 250ms, before Fixation offset
                    st1 = t #setting up sampling rate
                    if eyelink.sample_update(st0,st1):
        ...
        with PyCallGraph(output=GraphvizOutput()):
            ...
            while continueRoutine and IAPSClock.getTime() < blank_offset:
                ...
                if t >= 0.005 and iaps_display.status == STARTED: #collect samples 5ms after stimulus onset
                    while Window:
                        if t - st0 > 1.800:
                            print('diff between samples %s'%(st1-st0))
                            ps = eyelink.pupil_sample()
                            lps.append(ps)
                            st0 = st1 #debug time between samples
                        if t - ct0
    
                #eyelink-iaps_cue* updates
                if t >= 0.005 and iaps_display.status == STARTED: #run while stimulus is being displayed
                    ct1 = t
                    if eyelink.cue_update(ct0,ct1): #update cue display in intervals of 200 msec
                        ...
                        if len(lps) > rw: #start moving average after minimum samples (=rw)
                            old_cue = eyelink.pupil_cue(lps,pb_min,pb_max)
                            lps1 = len(lps)
                            print('color = %s'%(old_cue))#debug                    
                            print('total samples = %s' %(lps1))#debug

                            print('samples/cue = %s'%(lps1-lps0))
                            print('moving average = %s'%(p_ma))#debug
                            print('min = %s max = %s'%(pb_min,pb_max))#debug
                            print('cue time = %s' %(ct0))#debug
                            print('time = %s'%(t))#debug
                            print('time between cues = %s'%(ct1 - ct0))#debug
                            ct0 = ct1 #debug time between updates
    ...
    for thisBreak_loop in Break_loop:
        ...
        Break_end = Break_end + 1 #trial count
        if Break_end == 2:
