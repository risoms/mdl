...
class libeyelink():
    ...
    def pupil_sample_new(self):
        ...
        if(s != None): # Gets the gaze position of the latest sample    
            ...
            ps_old = ps
            #if first sample            
            if first_loop: #collect first sample               
                first_loop = False
                ps_n2 = ps_n1
                ps_n1 = ps
                return ps
            #calculating derivative of pupil area (sampling rate=500)
            #moving average
            ps_d = (ps - ps_n2)/(2*500)
            self.moving_avg(lps)
            #check if blink has occured
            if ps_d < -0.01 * ma or ps_d > 0.01 * ma:
                ps = ps_n1
                #updating values            
                ps_n2 = ps_n1
                ps_n1 = ps 
...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        with PyCallGraph(output=GraphvizOutput()):
            ...
            frameN = -1
            #pupil
            pb_min,pb_max,pb_mean,pb_stdev = eyelink.min_max(lps) #collect baseline min max mean and stdev (debug)
            ...
            lps_old=[] #old ps - before removing blinks
            lps_d=[] #change in ps
            ...
            while continueRoutine and IAPSClock.getTime() < blank_offset:
                ...
                if IAPSClock.getTime() >= 0 and iaps_display.status == STARTED: #collect samples 5ms after stimulus onset
                    while Window:
                        ...
                        if eyelink.sample_time(st1,st0): #if time between samples > 2 msec                      
                            ...
                            #old ps - before removing blinks
                            lps_old.append(ps_old)
                            lps_d.append(ps_d)
                ...
                if IAPSClock.getTime() >= 0.250 and iaps_display.status == STARTED: #run while stimulus is being displayed
                    ...
                    Total_Samples = len(lps) #sample size after artifact clearing
        ...
        with open('_data\gaze\%s.csv'%(subno), 'a') as cross_val:
            ...
            if header:
                s_header=["block","trial","eyelink_timestamp","psychopy_timestamp","new_pupil_sample","pupil_running_avg","pupil_baseline_min","pupil_baseline_mean","pupil_baseline_max","old_pupil_sample","pupil_area_velocity","cpu","ram"]
                writer.writerow(s_header)
                header=False
            writer.writerows(itertools.izip(lblock,ltrial,ltt,lpp_t,temp_lps,lpma,lpb_min,temp_lpb,lpb_max,lps_old,lps_d,cpu_pc,ram_pc))
