...
class libeyelink():
    ...
    def moving_avg(self,count,lst):
        ...
        rm = pandas.rolling_mean(df,count)[count-1:] #moving average
        ma = rm['samples'][rm.index[-1]] #gets last pupil sample
        return ma
        
    def sample_time(self,s_1,s_0):  #collect new sample only if more than 1.9000 msec has gone by since old sample
        su = (s_1 - s_0) >= .0019
        return su
    
    def cue_time(self,c1,c0): #create new cue only if more than 2000 msec has gone by since old sample
        cu = (c1 - c0) >= 0.2000
        return cu
...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        while Fixation:
            ...
            #if drift correct failed twice run calibration
            if DC >=2:
                eyelink.calibrate()
                DC = 0
        ...
        with PyCallGraph(output=GraphvizOutput()):
            ...
            while continueRoutine and IAPSClock.getTime() < blank_offset:
                ...
                if IAPSClock.getTime() >= 0.005 and iaps_display.status == STARTED: #collect samples 5ms after stimulus onset
                    while Window:
                        ...
                        if eyelink.sample_time(st1,st0): #if time between samples > 1.800 msec                      
                            ...
                            lps.append(ps)
                            st0 = st1
                        if eyelink.cue_time(ct1,ct0): #if time between cues > 200 msec
                            ct0 = ct1
                            sample_raw = len(lps) - lps0 #sample size before artifact clearing
                            break
                ...
                if IAPSClock.getTime() >= 0.005 and iaps_display.status == STARTED: #run while stimulus is being displayed
                    ...
                    COUNT = lps1-lps0
                    if ((sample_raw - COUNT)/COUNT) > .5: #artifact clearing - if more than 50% difference, recollect samples
