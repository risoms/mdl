...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        left = x_size/2 - 100 #left bound
        top = y_size/2 - 100 #top bound
        right = x_size/2 + 100 #right bound
        bottom = y_size/2 + 100 #bottom bound
        box_window = 200 #200px
        ...
        endtime = -1
        currenttime = -1
        pylink.getEYELINK().sendCommand("draw_box %s %s %s %s %s" %(x_size/2 - 50, y_size/2 - 50, x_size/2 + 50, y_size/2 + 50,  7))        
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if fixation_cross.status == STARTED and t >= (0.0 + (8-win.monitorFramePeriod*0.75)): #most of one frame period left
