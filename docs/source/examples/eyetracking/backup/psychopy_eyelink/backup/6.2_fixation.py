...
class libeyelink():
    ...
    def moving_avg(self,lst):
        lst = [x for x in lst if x != -1 and x != 0] #removing samples with value of 0 and -1 before converting into DataFrame
...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        ltb=[]
        t0=0
        rw = 100 #running average window (appox 5 samples/ms * 15ms window)
        while Fixation:
            # update component parameters for each repeat
            inbox = False
            Finished = False
        ...
        stim_size = im.size[0]+1,im.size[1]+4
        # keep track of which components have finished
        IAPSComponents = []
        ...
        while continueRoutine and IAPSClock.getTime() < 7:
            ...
            #eyelink-pupil dilation stimulus presentation
            if t < 2: #baseline duration 2000ms, after stimulus onset
                t1 = eyelink.tracker_time() #prevent duplicate samples due to difference in refresh rate (Eyelink 500Hz)
                if (t1 - t0) > 1500: #collect sample only if more than 1.5msec (=1500 microsec) has gone by
                    ps = eyelink.pupil_sample()
                    lps.append(ps)
            ...
            #allow moving average collection
            if t >=2 and collect_ma:
                pma = eyelink.moving_avg(lps)#create moving average
                print('pma = %s' %(pma))
                collect_ma = False #end moving average collection

            #eyelink-iaps_cue* updates
            if t >= 2 and iaps_cue.status == NOT_STARTED:
                eyelink.pupil_cue(pma,pb_min,pb_max)
                
            if iaps_cue.status == STARTED and t >= (5-win.monitorFramePeriod*0.75): #most of one frame period left
