...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.83.04), April 08, 2016, at 09:57
...
size_fix = 200
fixbar = size_fix/4
xbdr = [((x_size/2)-fixbar), ((x_size/2)+fixbar)]
ybdr = [((y_size/2)-fixbar),((y_size/2)+fixbar)]
fixdone = False
fixTime = 0.5
DURATION = 2     
RIGHT_EYE = 1
LEFT_EYE = 0
drift_screen = visual.ImageStim(win, name='drift_screen',
...
tracker.calibrate()
eye_used = pylink.getEYELINK().eyeAvailable()
...
for thisBlock_loop in block_loop:
    ...
    for thisTrial_Loop in Trial_Loop:
        ...
        #pylink.getEYELINK().sendCommand("draw_box  %d %d %d %d" %(xbdr[0],ybdr[0],xbdr[1],ybdr[1]))
        ...
        while 1:
            #Writes out a time out message if no response is made within 2000msec window
            if (time.clock() - startTime) > DURATION:
                pylink.getEYELINK().sendMessage("Fixfail_DC")
                #'Wait for Eyemovement buffer to fill up before we start collecting sample data
            ...
            fixWindow = time.clock()
            # check if within fixation location
            if xbdr[0] < gaze_position[0] < xbdr[1] and ybdr[0] < gaze_position[1] < ybdr[1]:
                # Have we been in the box long enough?
                if (time.clock() - fixWindow) > fixTime:
                    pylink.getEYELINK().sendMessage("FixationOffset")
                    break
                else:
        ...
        # keep track of which components have finished
        IAPSComponents = []
        IAPSComponents.append(fixation_cross)
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if t >= 2 and iaps_display.status == NOT_STARTED:
                # keep track of start time/frame for later
            ...
            if iaps_display.status == STARTED and t >= (7-win.monitorFramePeriod*0.75): #most of one frame period left
                iaps_display.setAutoDraw(False)
            
            # *blank* updates
            if t >= 7 and blank.status == NOT_STARTED:
                # keep track of start time/frame for later
                blank.tStart = t  # underestimates by a little under one frame
                blank.frameNStart = frameN  # exact frame index
                blank.setAutoDraw(True)
            if blank.status == STARTED and t >= (9-win.monitorFramePeriod*0.75): #most of one frame period left
        ...
        # Send fixation onset time
        offset = int((t-fixation_cross.tStart) * 1000)
        msg = str(offset) + " Fixation_Onset"
