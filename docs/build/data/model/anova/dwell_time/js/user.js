...
function saveFileAs(data) {
    var textBlob = new Blob([data.replace(/<\/?[^>]+(>|$)/g, "")], {type:'text/x-R'});
    var file = document.createElement("a");
    file.download = 'model.R';
    file.innerHTML = "Download File";
    file.href = window.URL.createObjectURL(textBlob);
    file.click();
}
// tinymce
function getTinyMCE(title){
	//add save button on right-click
	tinymce.PluginManager.add('save2', function (editor) {
		editor.ui.registry.addMenuItem('save2', {
			icon: 'save',
			text: 'save',
			tooltip: 'cut',
			button: '',
		...
		editor.ui.registry.addContextMenu('image', {
			update: function (element) {
				return !element.src ? 'sa' : 'image';
			}
		});
	});
	//override
	tinymce.overrideDefaults({
		plugins: ["codesample", "save", "save2", 'fullscreen'],
		toolbar: 'cut copy paste | selectall | save | fullscreen', //menu
		contextmenu: 'cut copy paste | selectall | save2'
	});
	//initiate
	tinyMCE = tinymce.init({
		...
		height: '800',
		menubar: false,
		toolbar: 'undo cut copy paste | selectall | save | fullscreen', //menu
		contextmenu: 'cut copy paste | selectall save', //right-click
		icons: '../semeon',
		font_formats: 'semeon',
		theme_url: ('./css/tinymce/semeon').replace("\/\/", "\/"),
		skin_url:  ('./css/tinymce/semeon').replace("\/\/", "\/"),
		content_css : ['css/tinymce/semeon/content.min.css'],
		skin_css : ['css/tinymce/semeon/skin.mincss'],
		mobile: {
			//theme: 'silver'
			height: '200',
			theme: 'mobile',
...
function getLogitTable(){
	...
	//normal
		...
		logit_table = $('#table').DataTable({
			...
			columns: [
				...
				{data: "B", className:"desktop"},
				{data: "SE", className:"desktop"},
				{data: "t", className:"desktop"},
				{data: "df", className:"desktop"},
				{data: "Pr(>|t|)", className:"all"},
				{data: "OR", className:"all"},
				{data: "95% CI", className:"all"},
			],
			// columnDefs: [
			...
			var data = this.data();
			if(source=="anova"){
				if ((data[data.length-1] < 0.06) && (data[data.length-1] > 0.01)){
					data[data.length-1] = '<b>' + data[data.length-1] + '</b>';
					this.data(data);
				} else if (data[data.length-1] < 0.0001) {
					data[data.length-1] = '<b>< 0.0001</b>';
					this.data(data);
				} else {
					data[data.length-1] = data[data.length-1];
					this.data(data);
	...
	//hover
	if((source!="demographic") && (source!="device") && (source!="logit")&& (source!="onset") && (source!="anova") && (source!="definitions") && (source!="task") && (source!="summary")){	
